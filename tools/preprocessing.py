...
def clean_df(raw_df):
    raw_df.columns = ['index', 'Timestamp', 'Username', 'Embedded_text',
                      'Likes', 'Comments', 'Retweets', 'Quotes', 'Tweet URL']
    raw_df = raw_df.drop_duplicates(
        subset=['index', 'Tweet URL', 'Embedded_text'])
    ...
    raw_df['Comments'] = raw_df['Comments'].astype('int')
    raw_df['Retweets'] = raw_df['Retweets'].astype('int')
    raw_df['Likes'] = raw_df['Likes'].astype('int')
    raw_df['Quotes'] = raw_df['Quotes'].astype('int')
    raw_df['Emojis'] = raw_df['Embedded_text'].apply(create_emoji_ls)
    raw_df['Embedded_text'] = raw_df['Embedded_text'].apply(remove_emoji)
    return raw_df
